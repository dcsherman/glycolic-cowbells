...
EXTERNAL_DRIVE_DIRECTORY = 'IASION\\Archive_Backups\\md_backup_{0}'
...
GOOGLE_DRIVE_DIRECTORY = 'C:/Users/Csherman/Google Drive/Archive'
MAIN_BACKUP_DIRECTORY = 'C:/Users/Csherman/Desktop/Archive_Backups/md_backup_{0}'
EXTERNAL_DRIVE_DIRECTORY = 'IASION/My Files/Archive_Backups/md_backup_{0}'

Instead of converting a date to a string (which may also be locale dependent) and then replacing characters in it, better to use strftime to generate exactly the format that you want:
...
I think it's also good to use more specific names for the directory variables to clarify which one is a source and destination.

I typically create wrapper methods around anything that is specific to the target platform [aka, Windows™, Mac™, Linux™, Android™, etc]. I might use method names that look like setGoogleDriveDir( ), getExternDriveDir( ), learnBackupDir( ), etc. These methods may simply return the platform specific constants, but they may also do something complex to supply the needed value using platform specific components. 

For example, adding a system call that discovers the specific platform, then switch-case logic to compute appropriate return values. The better to anticipate the requirement to run these scripts on multiple platforms in future. 
